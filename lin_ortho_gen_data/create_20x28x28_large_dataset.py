...
pressure_ind_list = np.array([(i+1)*10 - 2 for i in range(n_loads)])
pressures = load[pressure_ind_list + 1]

new_dataset = np.zeros((5*n_data, n_loads, n_c*n_c, 3), dtype=np.single)
for j in range(5):
    ...
    for i in range(n_data):
        for kind, k in enumerate(pressure_ind_list):
            # fit rbf and evaulte
            dxh = invbubble.rbf_function(xy_data[0, :, :2], data[i, k, :, 0], xyh)
            dyh = invbubble.rbf_function(xy_data[0, :, :2], data[i, k, :, 1], xyh)
            dzh = invbubble.rbf_function(xy_data[0, :, :2], data[i, k, :, 2], xyh)
...
new_dataset = new_dataset.reshape((5*n_data*n_loads*n_c*n_c, 3))
scaler = MinMaxScaler()
...
# save the data
with h5py.File('10k20x28x28data.hdf5', 'w') as f:
    dset = f.create_dataset('10k20x28x28data', data=new_dataset,
                            compression="gzip")
